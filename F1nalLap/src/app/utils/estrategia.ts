...
export const DEGRADACION_POR_FASE: Record<Compuesto, Record<'inicial' | 'media' | 'final', [number, number]>> = {
  blandos: {
    inicial: [0.1, 0.15],
    media: [0.05, 0.1],
    final: [0.15, 0.2],
...
export function ajustarCompuesto(compuesto: Compuesto, vueltas: number): Compuesto {
  if (compuesto === 'medios' && vueltas < 18) return 'blandos';
  if (compuesto === 'duros' && vueltas <= 25) return 'medios';
  return compuesto;
}

export function calcularTiemposBase(circuito: { tiempo_inicial: number }): Record<Compuesto, number> {
...
export function formatearEstrategia(estrategia: [Compuesto, number][]): string {
  ...
  for (const [compuesto, vueltas] of estrategia) {
    ...
    salida.push(`${compuesto.charAt(0).toUpperCase() + compuesto.slice(1)}, Usar entre la vuelta ${inicio} y ${fin}`);
...
export function normalizarEstrategia(estrategia: [Compuesto, number][]): [Compuesto, number][] {
...
export function estrategiasDiferentes(
  ...
  for (let i = 0; i < est1.length; i++) {
    ...
    if (comp1 !== comp2 || Math.abs(vueltas1 - vueltas2) > umbralVueltas) return true;
  }
...
export function simularEstrategia(
  ...
  for (const [compuesto, vueltas] of estrategia) {
    ...
    if (vueltas > VIDA_UTIL[compuestoAjustado]) {
      throw new Error(`El compuesto ${compuestoAjustado} no puede durar más de ${VIDA_UTIL[compuestoAjustado]} vueltas.`);
    }

    const tiempoBase = tiemposBase[compuestoAjustado];
    const tiempoStint = calcularTiempoStint(compuestoAjustado, vueltas, tiempoBase);
...
export function generarEstrategias(vueltasTotales: number): [Compuesto, number][][] {
  ...
  for (const compuesto1 in VIDA_UTIL) {
    for (const compuesto2 in VIDA_UTIL) {
      if (compuesto1 !== compuesto2) {
        for (let vueltas1 = MINIMOS_VUELTAS[compuesto1 as Compuesto][0]; vueltas1 <= Math.min(MINIMOS_VUELTAS[compuesto1 as Compuesto][1], vueltasTotales); vueltas1++) {
          const vueltas2 = vueltasTotales - vueltas1;
          if (vueltas2 >= MINIMOS_VUELTAS[compuesto2 as Compuesto][0] && vueltas2 <= MINIMOS_VUELTAS[compuesto2 as Compuesto][1]) {
            estrategias.push([[compuesto1 as Compuesto, vueltas1], [compuesto2 as Compuesto, vueltas2]]);
          }
        }
      }
    }
  ...
  for (const compuesto1 in VIDA_UTIL) {
    for (const compuesto2 in VIDA_UTIL) {
      if (compuesto1 !== compuesto2) {
        for (let vueltas1 = MINIMOS_VUELTAS[compuesto1 as Compuesto][0]; vueltas1 <= Math.min(MINIMOS_VUELTAS[compuesto1 as Compuesto][1], vueltasTotales); vueltas1++) {
          for (let vueltas2 = MINIMOS_VUELTAS[compuesto2 as Compuesto][0]; vueltas2 <= Math.min(MINIMOS_VUELTAS[compuesto2 as Compuesto][1], vueltasTotales - vueltas1); vueltas2++) {
            const vueltas3 = vueltasTotales - vueltas1 - vueltas2;
            if (vueltas3 >= MINIMOS_VUELTAS[compuesto2 as Compuesto][0] && vueltas3 <= MINIMOS_VUELTAS[compuesto2 as Compuesto][1]) {
              estrategias.push([[compuesto1 as Compuesto, vueltas1], [compuesto2 as Compuesto, vueltas2], [compuesto2 as Compuesto, vueltas3]]);
            }
          }
        }
      }
  ...
  for (let vueltasBlando = MINIMOS_VUELTAS['blandos'][0]; vueltasBlando < Math.min(VIDA_UTIL['blandos'], vueltasTotales); vueltasBlando++) {
    for (let vueltasMedio = MINIMOS_VUELTAS['medios'][0]; vueltasMedio < Math.min(VIDA_UTIL['medios'], vueltasTotales - vueltasBlando); vueltasMedio++) {
      const vueltasDuro = vueltasTotales - vueltasBlando - vueltasMedio;
      if (vueltasDuro >= MINIMOS_VUELTAS['duros'][0] && vueltasDuro <= VIDA_UTIL['duros']) {
        estrategias.push([
